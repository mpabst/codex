...
const collectResult = (query: Processor, ary: Bindings[]) => (b: Bindings) => {
...
describe('Query', () => {
  it('unify one quad', () => {
    ...
    const query = new Processor(store, source)
  ...
  describe('basic conjunction performance', () => {
    ...
    it('is fast', () => {
      let results = 0
      new Processor(engine, query).evaluate(() => results++)
  ...
  describe('basic rule invocation', () => {
    it('socrates is mortal', () => {
      ...
      const query = new Processor(store, {
        type: 'Pattern',
